...
-- pg函数：定期执行，返回影响的记录数
-- 对已完成的流程且创建时间为三个月前的流程变量数据，
-- 按创建年份${year}和分片大小提取到历史表'act_hi_varinst_${year}_${index}'
-- 删除act_hi_varinst表中相应数据（注：未实现）
create or replace function dump_act_hi_varinst() returns integer as
$body$
declare
dumpdate date := (current_date - interval '3 month');
table_index integer := cast(extract(month from dumpdate) as integer) % 4; 
...
where p.end_time_ is not null and p.start_time_ < dumpdate);
begin
	if (itemNum > 0) 
	then
		if (isExist = 0)
		then 
			...
			where p.end_time_ is not null and p.start_time_ < to_date(''' || dumpdate || ''',''YYYY-MM-DD'')' ||
			');';
